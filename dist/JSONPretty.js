...
var JSONPretty = /** @class */ (function (_super) {
    ...
    JSONPretty.prototype.render = function () {
        ...
        return (React.createElement("div", __assign({}, rest, { dangerouslySetInnerHTML: { __html: "<pre class=\"" + themeClassName + "\"" + getStyle('main', theme, styles) + ">" + this._pretty(theme, obj, replacer, pointerFormatter, +space, styles) + "</pre>"
            } })));
    };
    // JSON =》 HTML转换器
    JSONPretty.prototype._pretty = function (theme, obj, replacer, pointerFormatter, space, styles) {
    ...
    // 格式化函数
    JSONPretty.prototype._replace = function (theme, styles, pointerFormatter, match, ind, key, val, tra) {
        var spanEnd = '</span>';
        ...
        if (val) {
            if (val === 'true' || val === 'false') {
                ...
                var valDisplay = pointerFormatter ? pointerFormatter(key, val) : val;
